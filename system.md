...
- **Global Protection & Context Injection**: All endpoints (except explicitly marked `@Public()` routes) are guarded by `ClientAuthGuard`, which validates the JWT signature and expiration. Valid requests automatically inject the resolved `Client` entity into the route handler via the `@CurrentClient()` decorator. The tenant lookup inside the `JwtStrategy` is fully cached via Redis to prevent repeated PostgreSQL round-trips for every authenticated request, with automated cache invalidation logic handling client suspension or modification.
