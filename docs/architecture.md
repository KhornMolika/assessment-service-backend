...
The platform is secured by a robust, two-tier architecture: a static API key for administrative operations and an **OAuth2 Client Credentials Grant** architecture designed for machine-to-machine integrations by tenants.

- **Administrative Security (Super Admin)**: Administrative operations on clients (such as creating a client, rotating secrets, suspending, and activating) are strictly protected by a `SuperAdminGuard`. This guard intercepts requests and securely validates the `x-admin-api-key` header against the system's `ADMIN_API_KEY` environment variable using constant-time comparison to prevent timing attacks.
- **Client Provisioning**: Clients (tenants) are provisioned via `POST /api/v1/clients` (Admin-only). The system generates a UUID `clientId` and a 64-character cryptographically secure `clientSecret`. The raw secret is returned **exactly once** in the response and is never stored in plaintext.
...
- **Token Exchange**: Clients exchange their credentials for a short-lived JSON Web Token (JWT) via the `POST /api/v1/auth/token` endpoint. The endpoint verifies the Argon2id hash and the client's `isActive` status. This specific endpoint is protected by an `AuthThrottlerGuard` providing strict per-clientId rate limiting (with IP fallback) to protect against credential stuffing and hash-computation DDOS.
- **Tenant Self-Service**: Tenants can manage their own profiles securely using their JWT token. The endpoints `GET /api/v1/clients/me` and `PATCH /api/v1/clients/me` allow tenants to read their profile and update limited configurations (like `webhookUrl` and `allowedOrigins`) without needing administrative access.
- **Global Protection & Context Injection**: All standard endpoints (except explicitly marked `@Public()` routes and the Admin-only routes) are guarded by `ClientAuthGuard`, which validates the JWT signature and expiration. Valid requests automatically inject the resolved `Client` entity into the route handler via the `@CurrentClient()` decorator. The tenant lookup inside the `JwtStrategy` is fully cached via Redis to prevent repeated PostgreSQL round-trips for every authenticated request, with automated cache invalidation logic handling client suspension or modification.
- **Global Rate Limiting**: General read and write endpoints are governed by a multi-tiered, configuration-driven throttler (using `@nest-lab/throttler-storage-redis`) configured by environment variables (e.g. `THROTTLE_READ_LIMIT`, `THROTTLE_WRITE_BURST_LIMIT`). Throttled requests immediately return `429 Too Many Requests` with standard `Retry-After` headers.
