...
Implement a Provider-Agnostic AI Adapter layer. The background Bull queue worker communicates with an interface (`IAProvider`), which is resolved at runtime based on the `AI_PROVIDER` environment variable.
