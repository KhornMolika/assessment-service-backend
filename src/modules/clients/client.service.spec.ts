...
describe('ClientService', () => {
  ...
  describe('create', () => {
    it('should throw ConflictException if slug exists', async () => {
      clientRepositoryMock.findBySlug.mockResolvedValue({ id: '1', slug: 'test' });
      await expect(service.create({ slug: 'test', name: 'Test' })).rejects.toThrow(ConflictException);
    });
    ...
    it('should create successfully and return raw secret', async () => {
      ...
      clientRepositoryMock.create.mockReturnValue({ id: '1', slug: 'test', clientSecretHash: 'hashed_secret' });
      clientRepositoryMock.save.mockResolvedValue({ id: '1', slug: 'test', clientSecretHash: 'hashed_secret' });
  ...
  describe('update', () => {
    it('should update client and invalidate cache', async () => {
      clientRepositoryMock.findOne.mockResolvedValue({ id: '1', clientId: 'c1' });
      clientRepositoryMock.save.mockResolvedValue({ id: '1', clientId: 'c1', name: 'New' });
  ...
  describe('rotateSecret', () => {
    it('should rotate secret and invalidate cache', async () => {
      ...
      clientRepositoryMock.save.mockResolvedValue({ id: '1', clientId: 'c1', clientSecretHash: 'new_hash' });
  ...
  describe('setActive', () => {
    it('should set active and invalidate cache', async () => {
      clientRepositoryMock.findOne.mockResolvedValue({ id: '1', clientId: 'c1' });
      clientRepositoryMock.save.mockResolvedValue({ id: '1', clientId: 'c1', isActive: false });
  ...
  describe('verifySecret', () => {
    ...
    it('should return null if client is not active', async () => {
      clientRepositoryMock.findByClientId.mockResolvedValue({ id: '1', isActive: false });
      const result = await service.verifySecret('c1', 'raw');
      expect(result).toBeNull();
    });

    it('should return null if secret is invalid', async () => {
      clientRepositoryMock.findByClientId.mockResolvedValue({ id: '1', isActive: true, clientSecretHash: 'hash' });
