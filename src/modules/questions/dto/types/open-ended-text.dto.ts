...
export class OpenEndedTextOptionsDto {
  @ApiProperty({
    example: 0,
    description: 'Minimum number of words expected',
    minimum: 0,
  })
  @IsInt()
  @Min(0)
...
export class OpenEndedTextAnswerDto {
  @ApiProperty({
    ...
    description: 'Model or reference answer for grading or reference',
  })
  @IsString()
  @IsNotEmpty()
  ...
  @ApiProperty({
    example: ['progressive', 'scalable', 'server-side'],
    description: 'List of key points expected in a correct answer',
  })
  @IsArray()
  @IsString({ each: true })
