import { BadRequestException, Injectable, UnauthorizedException, InternalServerErrorException, Logger, HttpException } from '@nestjs/common';
...
export class AuthService {
  ...
  async token(dto: TokenRequestDto): Promise<TokenResponseDto> {
    try {
      ...
      this.logger.error(`Failed to generate token: ${(error as Error).message}`, (error as Error).stack);
