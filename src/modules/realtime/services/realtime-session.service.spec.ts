...
describe('RealtimeSessionService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: AssessmentParticipantRepository, useValue: participantsMock },
  ...
  describe('startSession', () => {
    it('should throw if assessment not found', async () => {
      ...
      await expect(service.startSession('a1')).rejects.toThrow(NotFoundException);
    });

    it('should throw if not published', async () => {
      assessmentsMock.findById.mockResolvedValue({ status: AssessmentStatus.DRAFT });
      await expect(service.startSession('a1')).rejects.toThrow(BadRequestException);
    });

    it('should throw if session already active', async () => {
      assessmentsMock.findById.mockResolvedValue({ status: AssessmentStatus.PUBLISHED });
  ...
  describe('joinRoom', () => {
    it('should throw if session not found', async () => {
      redisMock.getSession.mockResolvedValue(null);
      await expect(service.joinRoom('a1', 's1', 'p1', 'participant', 'John')).rejects.toThrow(NotFoundException);
    });

    it('should join and update host', async () => {
      redisMock.getSession.mockResolvedValue({});
      redisMock.getMembers.mockResolvedValue([{ role: 'participant', participantId: 'p1', name: 'John' }]);
      const res = await service.joinRoom('a1', 's1', 'p1', 'host', 'Host');
      
      expect(redisMock.addMember).toHaveBeenCalled();
      expect(redisMock.updateSession).toHaveBeenCalledWith('a1', { hostSocketId: 's1' });
  ...
  describe('startQuestion', () => {
    it('should throw if session not found', async () => {
      redisMock.getSession.mockResolvedValue(null);
      await expect(service.startQuestion('a1', 's1')).rejects.toThrow(NotFoundException);
    });

    it('should throw if not host', async () => {
      redisMock.getSession.mockResolvedValue({ hostSocketId: 'host1' });
      await expect(service.startQuestion('a1', 's1')).rejects.toThrow(ForbiddenException);
    });

    it('should start specific question', async () => {
      redisMock.getSession.mockResolvedValue({ hostSocketId: 's1', currentQuestionIndex: -1 });
      assessmentQuestionsMock.findByAssessment.mockResolvedValue([
        { id: 'q1', questionSnapshot: { type: 'SINGLE_CHOICE', options: [{id: 'opt1', text: 'Opt1'}] } }
      ]);

      const res = await service.startQuestion('a1', 's1', 'q1');
      expect(res.q.id).toBe('q1');
      expect(redisMock.updateSession).toHaveBeenCalledWith('a1', expect.objectContaining({ currentQuestionId: 'q1' }));
    });

    it('should start next question', async () => {
      redisMock.getSession.mockResolvedValue({ hostSocketId: 's1', currentQuestionIndex: 0 });
      assessmentQuestionsMock.findByAssessment.mockResolvedValue([
    ...
    it('should throw if no more questions', async () => {
      ...
      await expect(service.startQuestion('a1', 's1')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('submitAnswer', () => {
    it('should throw if session not found', async () => {
      redisMock.getSession.mockResolvedValue(null);
      await expect(service.submitAnswer('a1', 'p1', 'q1')).rejects.toThrow(NotFoundException);
    });

    it('should throw if not active', async () => {
      redisMock.getSession.mockResolvedValue({ status: 'waiting' });
      await expect(service.submitAnswer('a1', 'p1', 'q1')).rejects.toThrow(BadRequestException);
    });

    it('should throw if wrong question', async () => {
      redisMock.getSession.mockResolvedValue({ status: 'active', currentQuestionId: 'q2' });
      await expect(service.submitAnswer('a1', 'p1', 'q1')).rejects.toThrow(BadRequestException);
    });

    it('should store answer', async () => {
      redisMock.getSession.mockResolvedValue({ status: 'active', currentQuestionId: 'q1' });
  ...
  describe('endQuestion', () => {
    it('should throw if no active question', async () => {
      ...
      await expect(service.endQuestion('a1')).rejects.toThrow(BadRequestException);
    });

    it('should calculate stats and add scores', async () => {
      redisMock.getSession.mockResolvedValue({ currentQuestionId: 'q1', currentQuestionIndex: 0 });
      assessmentQuestionsMock.findByAssessment.mockResolvedValue([
        { id: 'q1', points: 10, questionSnapshot: { type: 'SINGLE_CHOICE', correctAnswer: { optionId: 'opt1' } } }
      ]);
      ...
      expect(redisMock.addScore).toHaveBeenCalledWith('a1', 'p1', expect.any(Number));
  ...
  describe('getRankData', () => {
    it('should return top 5', async () => {
      redisMock.getTopScores.mockResolvedValue([{ rank: 1, participantId: 'p1', score: 100 }]);
  ...
  describe('endSession', () => {
    it('should flush to postgres and clear redis', async () => {
      jest.useFakeTimers();
      redisMock.getAllScores.mockResolvedValue([{ participantId: 'p1', score: 100, rank: 1 }]);
      redisMock.getName.mockResolvedValue('John');
      assessmentsMock.findById.mockResolvedValue({ clientId: 'c1' });
      settingsMock.findByAssessment.mockResolvedValue({ passMark: 50, gradeLabels: [{min: 50, name: 'Pass'}] });
      assessmentQuestionsMock.findByAssessment.mockResolvedValue([
        { id: 'q1', points: 100, questionSnapshot: { type: 'SINGLE_CHOICE', correctAnswer: { optionId: 'opt1' } } }
      ]);
      redisMock.getAnswers.mockResolvedValue({
        p1: { choice: 'opt1' }
      });
      ...
      expect(redisMock.updateSession).toHaveBeenCalledWith('a1', { status: 'ended' });
