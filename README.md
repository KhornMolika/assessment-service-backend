...
* **Real-Time Instructor-Led Assessments (Kahoot-Style)**: A fully synchronized WebSocket and Redis-backed session engine for live, host-controlled exams. Features instant auto-grading for single/multiple choice, true/false, ordering, and matching questions. Includes dynamic connection resilience for participants joining late or recovering from dropped connections. The WebSocket API strictly follows a Kahoot-style architecture, allowing hosts to skip timers, trigger immediate answer reveals (`REVEAL_ANSWERS`), and automatically computes dynamic time-bonuses and Top 3 podium standings. When a session concludes, the exact live leaderboard and time-bonus scores are flushed from Redis to PostgreSQL to ensure full historical compatibility with the **Reports API**.
